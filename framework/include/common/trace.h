...
/* ==================== 可注入实例（严格依赖注入） ==================== */
typedef uint32_t (*TraceTimestampFn)(void* ctx);

typedef struct {
    TraceTimestampFn now;
    void* ctx;
} AegisTraceClock;

typedef struct {
    AegisRingBuffer ring;
    uint8_t buffer[TRACE_LOG_SIZE * sizeof(AegisTraceEvent)];
    AegisTraceClock clock;
    uint32_t fallback_tick;
 ...
 * @brief: 初始化追溯日志
 * @param log: 追溯日志实例
 * @param now_fn: 时间戳回调（可为NULL，使用fallback自增计数）
 * @param now_ctx: 时间戳回调上下文
 * @return: 错误码
 * @req: REQ-TRACE-001
...
/*
 * @brief: 获取系统时间戳（使用注入回调或fallback计数）
 * @param log: 追溯日志实例
 * @return: 系统滴答时间戳
 * @req: REQ-TRACE-005
